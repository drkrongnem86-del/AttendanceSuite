...
class LiveScreenState extends State<LiveScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        _status = '✅ Probe lúc ${DateFormat('HH:mm:ss').format(_lastProbeAt!)}: '
            '${result['online_count'] ?? 0}/${devs.length} online';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  void _toggleAutoProbe(bool enabled) {
    if (enabled) {
      _startAutoProbe();
    } else {
      _probeTimer?.cancel();
    ...
    return ListView(
      ...
      children: [
        ...
        Row(children: [
          ...
          ElevatedButton.icon(
            onPressed: () {
              _toggleAutoProbe(_probeTimer?.isActive != true);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: _probeTimer?.isActive == true ? Colors.orange.shade700 : const Color(0xFF455A64),
              foregroundColor: Colors.white,
              padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 12),
            ),
            icon: Icon(_probeTimer?.isActive == true ? Icons.stop_circle : Icons.play_circle, size: 16),
            label: Text(_probeTimer?.isActive == true ? 'Dừng' : 'Auto',
  ...
  Widget _header() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Spacer(),
        Text('Auto-probe ${_probeIntervalSec}s',
