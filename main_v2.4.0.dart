...
class AppState extends ChangeNotifier {
  ...
  // v2.4.0: PC-Relay mode (bypass Sophos VPN)
  // Khi bật: embedded server sẽ proxy các request ZK sang PC chạy EXE
  // (PC đã có VPN, đã vào được 172.16.x.x → truy cập được thiết bị)
  String relayServerUrl = '';       // ví dụ: http://192.168.1.100:8080 hoặc http://172.16.0.31:8080
  bool useRelayMode = false;        // toggle bật/tắt relay

  // Known BV PCs (dùng cho auto-discover)
  static const List<String> _knownBvPcs = [
    'http://172.16.0.31:8080',      // Secutime server
    'http://172.16.200.105:8080',   // ADMS proxy
    'http://172.16.254.202:8080',   // May 14 fw (ZKDB)
    'http://172.16.1.100:8080',     // Generic
  ...
  /// v2.4.0: Get known BV PC URLs for auto-discover
  List<String> get knownBvPcs => List.unmodifiable(_knownBvPcs);
  ...
  Future<void> init() async {
    ...
    relayServerUrl = prefs.getString('relay_server_url') ?? '';
    useRelayMode = prefs.getBool('relay_enabled') ?? false;

    // Try starting embedded server
    server = EmbeddedServer(
      port: serverPort,
      relayServerUrl: relayServerUrl,
      useRelayMode: useRelayMode,
    );
    try {
      await server.start();
      serverRunning = true;
  ...
  /// v2.4.0: Save relay config và restart server với config mới
  Future<void> setRelayConfig({String? url, bool? enabled}) async {
    if (url != null) relayServerUrl = url.trim();
    if (enabled != null) useRelayMode = enabled;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('relay_server_url', relayServerUrl);
    await prefs.setBool('relay_enabled', useRelayMode);
    // Update server's relay config without restart (hot-swap)
    server.setRelay(relayServerUrl: relayServerUrl, useRelayMode: useRelayMode);
    notifyListeners();
  }

  /// v2.4.0: Test relay connection (ping /api/status)
  Future<Map<String, dynamic>> testRelay(String url) async {
    try {
      final resp = await HttpClient()
          .getUrl(Uri.parse('$url/api/status'))
          .then((req) => req.close())
  ...
  /// v2.4.0: Auto-discover PC chạy EXE (scan known BV PCs)
  Future<Map<String, dynamic>> autoDiscoverRelay() async {
    for (final url in _knownBvPcs) {
      final r = await testRelay(url);
      if (r['ok'] == true) {
        return {'ok': true, 'url': url, 'data': r['data']};
      }
    }
    return {'ok': false, 'error': 'Không tìm thấy PC EXE nào trong danh sách BV'};
  }
...
class _AttendanceAppState extends State<AttendanceApp> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this);  // v2.4.0: lifecycle cho VPN auto-disconnect
    _state = AppState();
    _state.init().then((_) {
      if (mounted) setState(() {});
    });
  ...
  // v2.4.0: Hook vào lifecycle để trigger VpnManager.onAppPaused/onAppResumed
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.hidden:
        try {
          _state.vpn.onAppPaused();
        } catch (_) {}
        break;
      case AppLifecycleState.resumed:
        try {
          _state.vpn.onAppResumed();
        } catch (_) {}
        break;
...
class _TopBar extends StatelessWidget {
          ...
          return SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _row('PC-Relay:',
                  state.useRelayMode
                    ? '🟢 BẬT → ${state.relayServerUrl}'
                    : '⚪ Tắt (dùng VPN Sophos trực tiếp)',
                  color: state.useRelayMode ? Colors.cyanAccent : Colors.white60),
                ...
                const Text(
                  ...
                  'VPN Sophos: user=nemk → 113.176.81.193:8443\n'
                  'PC-Relay: bật để proxy sang PC chạy EXE (bypass Sophos).',
  ...
  Future<void> _toggleVpn(BuildContext context) async {
    ...
    if (vpn.status.connected) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('🛑 Đã ngắt VPN'), duration: Duration(seconds: 2)),
      );
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('⏳ Đang kết nối VPN Sophos (user=nemk → 113.176.81.193:8443)...'),
        duration: Duration(seconds: 2),
      ),
    );
    final res = await vpn.connectOneTap();
    if (!context.mounted) return;
    final tierLabel = switch (res.tier) {
      VpnTier.native => 'native',
      VpnTier.app => 'OpenVPN Connect',
      VpnTier.manual => 'manual (file)',
      VpnTier.unknown => '?',
    };
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${res.success ? "✅" : "❌"} [$tierLabel] ${res.message}'),
  ...
  Future<void> _restartServerWithFeedback(BuildContext context) async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('🔄 Đang restart server...'), duration: Duration(seconds: 1)),
    );
    await context.appState.restartServer();
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('✅ Server đã restart'), duration: Duration(seconds: 2)),
    );
  }

  Future<void> _confirmExit(BuildContext context) async {
    final state = context.appState;
    // Show dialog with 2 options: cancel + exit
    final shouldExit = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Server nhúng sẽ dừng. VPN (nếu đang chạy) sẽ tự đóng.\n'
          'Dữ liệu chấm công đã ghi vẫn được lưu trên máy ZK.',
          style: TextStyle(fontSize: 12),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('Hủy', style: TextStyle(color: Color(0xFF607D8B))),
          ),
          TextButton(
    ...
    if (shouldExit == true) {
      try { await state.vpn.disconnect(); } catch (_) {}
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: [
            _miniBtn(Icons.refresh, 'Restart Server', () => _restartServerWithFeedback(context)),
            _miniBtn(
              ...
              onSelected: (v) async {
                switch (v) {
                  case 'refresh_ips':
                    ...
                    break;
                  case 'restart':
                    await _restartServerWithFeedback(context);
                    break;
                  case 'vpn_settings':
            ...
            const Spacer(),
            _miniBtn(Icons.power_settings_new, 'Thoát (Hủy/Thoát)', () => _confirmExit(context),
