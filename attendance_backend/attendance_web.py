...
def load_devices():
    ...
    for line in lines:
        ...
        # v1.9: Allow all device types to be selected/fetched
        # For gateway/server, fetch will return empty (no ATTLOG)
        can_fetch = True
...
<div class="toolbar">
  ...
  <a href="/all-links" class="btn-outline" target="_blank" style="text-decoration:none;display:inline-flex;align-items:center">📋 All endpoints & pages →</a>
</div>
<div class="container">
  <div class="left-panel">
    ...
    <div class="filter-row">
      ...
      <label><input type="checkbox" id="showServer" onchange="applyFilter()"> Server</label>
      <label><input type="checkbox" id="showGateway" onchange="applyFilter()"> Gateway</label>
    </div>
...
function renderDeviceList() {
  ...
  const showSrv = document.getElementById('showServer').checked;
  const showGw  = document.getElementById('showGateway').checked;
  ...
  allDevices.forEach(function(d) {
    ...
    if (d.type === 'server' && !showSrv) return;
    if (d.type === 'gateway' && !showGw) return;
    if (search) {
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/' or path == '/index.html':
            ...
            all_links_path = os.path.join(os.path.dirname(__file__), 'all_links.html')
            if os.path.exists(all_links_path):
                with open(all_links_path, 'rb') as f:
                    content = f.read()
                self.send_response(200)
                self.send_header('Content-Type', 'text/html; charset=utf-8')
                self.end_headers()
...
# === Attach CVE-2023-3941 inject routes (v1.9+) ===
try:
    from inject_routes import attach_inject_routes
    attach_inject_routes(Handler)
    print('[OK] CVE-2023-3941 ATTLOG Inject routes attached')
except ImportError as e:
    print(f'[!] inject_routes.py not found: {e}')
