...
class EmbeddedApi {
  ...
  Future<List<Device>> getDevices() async {
    try {
      developer.log('GET $devicesUrl', name: _tag);
      final resp = await _client.get(Uri.parse(devicesUrl))
          .timeout(const Duration(seconds: 5));
      developer.log('Response ${resp.statusCode} bytes=${resp.body.length}', name: _tag);
      if (resp.statusCode == 200) {
        final d = jsonDecode(resp.body);
        if (d is Map && d['devices'] is List) {
          final list = (d['devices'] as List)
              .map((j) => Device.fromJson(j as Map<String, dynamic>))
              .toList();
          developer.log('Got ${list.length} devices', name: _tag);
          return list;
        }
        developer.log('No devices key in response: ${d.keys}', name: _tag);
      }
    } catch (e) {
      developer.log('getDevices ERROR: $e', name: _tag);
    }
    return [];
  }
