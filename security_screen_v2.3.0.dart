...
class _SecurityScreenState extends State<SecurityScreen> {
  ...
  Future<void> _scan() async {
    ...
    // v2.3.0 FIX: If scan returns empty list (no VPN / no network), KEEP existing
    // devices instead of wiping them. Otherwise UI shows "0 online, 0 offline"
    // when there are actually 26 devices that just can't be reached right now.
    List<Device> newDevices;
    String statusMsg;
    if (list.isEmpty && _devices.isNotEmpty) {
      // Preserve existing list, just update counts (offline = all)
      newDevices = _devices;
      online = 0;
      offline = _devices.length;
      statusMsg = '⚠️ Quét xong: 0 online, $_devices.length offline (không có VPN / mạng LAN)';
    } else {
      newDevices = list.map((j) => Device.fromJson(j as Map<String, dynamic>)).toList();
      statusMsg = '✅ Quét xong: $online online, $offline offline';
    }
