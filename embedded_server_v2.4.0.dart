...
class EmbeddedServer {
  ...
  EmbeddedServer({
    ...
    bool useRelayMode = false,
  })  : _relayServerUrl = relayServerUrl,
        _useRelayMode = useRelayMode;

  /// v2.4.0: Hot-swap relay config (không cần restart server)
  void setRelay({required String relayServerUrl, required bool useRelayMode}) {
    _relayServerUrl = relayServerUrl;
    _useRelayMode = useRelayMode;
    print('[EmbeddedServer] relay updated: use=$useRelayMode url=$relayServerUrl');
  }

  bool get isRelayEnabled => _useRelayMode && _relayServerUrl.isNotEmpty;
  ...
  /// v2.4.0: Proxy HTTP request sang PC-EXE khi relay mode bật.
  /// Trả về Response từ PC. Nếu relay không bật → trả null (để handler gọi logic local).
  Future<Response?> _proxyToRelay(Request req, {String? altPath}) async {
    if (!isRelayEnabled) return null;
    final path = altPath ?? '/api/${req.url.path.replaceFirst('api/', '')}';
    final targetUri = Uri.parse('$_relayServerUrl$path${req.url.hasQuery ? '?${req.url.query}' : ''}');
    final client = HttpClient()..connectionTimeout = const Duration(seconds: 10);
    try {
      final upstream = await client.openUrl(req.method, targetUri);
      // Forward headers (shelf req.headers là Map<String,String>)
      req.headers.forEach((k, v) {
        if (k.toLowerCase() != 'host' && k.toLowerCase() != 'content-length') {
          upstream.headers.add(k, v);
        }
      });
      // Forward body nếu có (POST/PUT)
      ...
      final body = await upstreamResp.fold<List<int>>([], (acc, chunk) {
        acc.addAll(chunk);
        return acc;
      });
      final headers = <String, String>{
        'content-type': upstreamResp.headers.value('content-type') ?? 'application/json; charset=utf-8',
        'x-relayed-from': _relayServerUrl,
      };
      return Response(upstreamResp.statusCode,
        body: body.isEmpty ? null : body,
        headers: headers);
    } catch (e) {
      print('[EmbeddedServer] relay proxy error: $e (target=$targetUri)');
      return _json({
        'ok': false,
        'relay': true,
        'error': 'Relay proxy failed: $e',
        'relay_url': _relayServerUrl,
        'hint': 'Kiểm tra PC có chạy EXE và reachable từ điện thoại',
      }, status: 502);
    } finally {
  ...
  Future<Response> _handleStatus(Request req) async {
    return _json({
      ...
      'relay_url': _relayServerUrl,
      'message': isRelayEnabled
          ? 'Local server OK (relay mode → $_relayServerUrl)'
          : 'Embedded server OK',
    });
  }
  ...
  Future<Response> _handleDevicesList(Request req) async {
    // v2.4.0: Relay mode → proxy sang PC-EXE
    final proxied = await _proxyToRelay(req);
  ...
  Future<Response> _handlePingAll(Request req) async {
    // v2.4.0: Relay mode → proxy sang PC-EXE (PC đã có VPN, reach được ZK)
    final proxied = await _proxyToRelay(req);
    if (proxied != null) return proxied;

    final stopwatch = Stopwatch()..start();

    // v2.2.8: Auto-detect VPN mode (like attendance_web.py _handle_ping_all).
    // Use NetworkInterface.list() — if local interface IPs are NOT in 172.16.x,
    // we're going through Sophos VPN tunnel → longer timeout needed.
    bool isVpn = false;
    String localIp = '';
    try {
      ...
      isVpn = nonVpnIps.isNotEmpty;
    } catch (_) {
      isVpn = true; // assume VPN (safer to use longer timeout)
    }

    // v2.4.0: VPN mode timeout tăng từ 3s → 5s (Sophos tunnel chậm)
    //         retries 3 → 5, workers 6 → 8 để chắc chắn reach được thiết bị
    final int effectiveTimeoutMs = isVpn ? 5000 : 2000;
    final int effectiveRetries = isVpn ? 5 : 2;
    final int maxParallel = isVpn ? 8 : 10;
    final String modeLabel = isVpn ? 'VPN' : 'LAN';

    // Run TCP pings in parallel with limited concurrency
    final attDevices = _devices
    ...
    Future<void> worker() async {
      while (true) {
        final i = cursor++;
        if (i >= attDevices.length) return;
        final d = attDevices[i];
        final (ok, latency, err) =
            await tcpPing(d.ip, 4370, timeoutMs: effectiveTimeoutMs, retries: effectiveRetries);
        d.online = ok;
    ...
    final workersN = maxParallel < attDevices.length ? maxParallel : attDevices.length;
