...
class EmbeddedServer {
  ...
  Future<void> start() async {
    ...
    try {
      // CRITICAL FIX: shelf_io.serve() binds + serves in ONE call (returns Future<HttpServer>)
      // Previous code was calling HttpServer.bind() then shelf_io.serve(handler, address, port)
      // which tried to bind AGAIN on same port - silently fails on Android.
      _startedAt = DateTime.now();
      _running = true;  // Set immediately so UI knows server is up
      _pid = pid;

      // shelf_io.serve binds the server and starts serving requests.
      // It returns a Future that completes when the server stops.
      // We use unawaited() so we can return from start() immediately.
  ...
  int _injectAttlogRecord(
    ...
    try {
      final beforeResult =
          db.select('SELECT COUNT(*) AS c FROM ATT_LOG');
      final before = beforeResult.first['c'] as int;
      // ATTLOG schema: id, User_PIN, Verify_Type, Verify_Time, Status, Work_Code_ID, Sensor_NO, Att_Flag, CREATE_ID, MODIFY_TIME, SEND_FLAG
      db.execute('''
